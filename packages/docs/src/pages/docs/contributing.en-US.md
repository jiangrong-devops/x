...
This guide will help you understand how to contribute to `antdv-next/x`. Please take a few minutes to read this guide before submitting an issue or pull request.
...
All our development work is done on [Github](https://github.com/antdv-next/x). Whether it's a pull request from a team member or a community contributor, all pull requests must go through the same review process.
...
To help you get started with your first attempt, we've marked some relatively easy-to-fix bugs and small features with [good first issues](https://github.com/antdv-next/x/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22). These issues can serve as a good first attempt.
...
The Antdv Next X Team will pay attention to all pull requests. We will review and merge your code, and may also ask you to make some changes or tell you why we cannot accept such changes.
...
2. Clone your forked repository locally: `git clone https://github.com/{owner}/x.git`.
3. Enter the project directory: `cd x`.
4. Install dependencies: `pnpm install`.
5. Run the development environment: `pnpm dev`.
6. Run code style check: `pnpm lint`.
7. Build the site: `pnpm build`.
8. Compile and build: `pnpm build:x`.
